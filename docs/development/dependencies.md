...
Read [ADR 0001](../adr/0001-dependencies-and-macos-platform.md) and [ADR 0003](../adr/0003-command-line-and-logging-dependencies.md) before proposing a package.
Document the required capability and explain why Foundation or an existing dependency boundary is insufficient.
...
Update the current dependency ADR license review so `uv run python scripts/check-licenses.py` can map every resolved identity.
